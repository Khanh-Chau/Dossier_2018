...
-- créer une vue qui s'appele export_visits
WITH
-- façon d'écrire les sous requêtes pour utilisation dans une requête SELECT plus étendue. 
    observers AS(
SELECT 
    v.id_base_visit,
    string_agg(roles.nom_role::text || ' ' ||  roles.prenom_role::text, ',') AS observateurs
    -- cette fonction string_agg concatène les valeurs des expressions de chaines
    -- et place les valeurs de séparateurs entre elles. 
...
--  on fait une sous requête pour avoir les utilisateurs 
),
perturbations AS(
SELECT 
    v.id_base_visit,
...
GROUP BY v.id_base_visit
--  sous requête pour avoir les perturbations 
)
...
-- permet de lister tous les résultats de la table gauche (donc ici c' la jointure entre t_base_sites, 
-- cor_site area et t_base_visits) même s'il n'y a pas de correspondance dans la deuxième table.
-- si on fait un join normal, ça va prendre que ceux qui sont en commun entre 2 tables
-- et c' pas ce qu'on veut. 
